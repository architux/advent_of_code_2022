...
carrying the most Calories. That way, even if one of those Elves runs out of snacks,
...
def get_calories_list(input_data: list) -> list[list[int]]:
    return [
        [int(n) for n in group]
        for key, group in groupby(input_data, lambda z: z == "")
        if not key
    ]


def get_calories_sums(
        calories_list: list, reverse: bool = True
) -> list[int]:
    return sorted([sum(cl) for cl in calories_list], reverse=reverse)


if __name__ == "__main__":
    test_data = get_calories_sums(
        get_calories_list(
            get_input_data("test_data.txt")
        )
    )
    prod_data = get_calories_sums(
        get_calories_list(
    ...
    # Part 1
    assert test_data[0] == 24000
    print(prod_data[0])

    # Part 2
    assert sum(test_data[:3]) == 45000
    print(sum(prod_data[:3]))
