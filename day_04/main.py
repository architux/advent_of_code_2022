...
def get_section_assignment_pairs(input_data: list[str]) -> list[list[list[int]]]:
    return [
        [
            list(int(j) for j in pair.split("-"))
...
def get_contained_pairs(input_data: list[list[list[int]]]):
    contained_pairs = []

    for first_item, second_item in input_data:
        first_set = set(range(first_item[0], first_item[1] + 1))
        second_set = set(range(second_item[0], second_item[1] + 1))

        if first_set.issubset(second_set) or second_set.issubset(first_set):
            contained_pairs.append([first_item, second_item])

    return contained_pairs


def get_overlapping_pairs(input_data: list[list[list[int]]]):
    overlapping_pairs = []
    ...
    for first_item, second_item in input_data:
        ...
        if len(set(first_set) & set(second_set)):
            overlapping_pairs.append([first_item, second_item])

    return overlapping_pairs


if __name__ == "__main__":
    test_data = get_section_assignment_pairs(
        get_input_data("test_data.txt")
    )
    prod_data = get_section_assignment_pairs(
        get_input_data("prod_data.txt")
    )

    # Part 1
    test_contained_pairs = get_contained_pairs(test_data)
    assert len(test_contained_pairs) == 2
    prod_contained_pairs = get_contained_pairs(prod_data)
    print(len(prod_contained_pairs))

    # Part 2
    test_overlapping_pairs = get_overlapping_pairs(test_data)
    assert len(test_overlapping_pairs) == 4
    prod_overlapping_pairs = get_overlapping_pairs(prod_data)
    print(len(prod_overlapping_pairs))
